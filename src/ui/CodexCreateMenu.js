...
export class CodexCreateMenu extends CodexBaseEntry {
    ...
    /**@type {HTMLElement} */ typeListDom;
    /**@type {HTMLElement} */ keyCrumbs;
    /**@type {HTMLElement} */ keyInput;
    /**@type {HTMLElement} */ bookCrumbs;
    ...
    async renderType() {
        ...
        for (const type of types) {
            const item = document.createElement('div'); {
                ...
                this.typeListDom.append(item);
            }
        }
        return this.doms[STAGE.TYPE];
    }
    async renderKey() {
        if (!this.doms[STAGE.KEY]) {
            const dom = document.createElement('div'); {
        ...
        this.keyCrumbs.textContent = [
            `Type: ${this.type}`,
        ].join(' | ');
        return this.doms[STAGE.KEY];
    }
    async renderBook() {
        if (!this.doms[STAGE.BOOK]) {
            const dom = document.createElement('div'); {
                ...
                const mesWrapper = document.createElement('div'); {
                    ...
                    const crumbs = document.createElement('div'); {
                        this.bookCrumbs = crumbs;
                        crumbs.classList.add('stcdx--crumbs');
                        mesWrapper.append(crumbs);
                    }

                    const typeList = document.createElement('div'); {
                        this.bookListDom = typeList;
                        typeList.classList.add('stcdx--typeList');
                        mesWrapper.append(typeList);
                    }
                    const actions = document.createElement('div'); {
